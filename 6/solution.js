...
const partOne = () => {
  ...
  return parsedInput.map(countQuestions).reduce((total, current) => total + current, 0)
}
...
const partTwo = () => {
  const countQuestions = group => group.reduce((questions, person) => {
    return intersection(new Set(person), questions)}, null
  ).size
  return parsedInput.map(countQuestions).reduce((total, current) => total + current, 0)
}
